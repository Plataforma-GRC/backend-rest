...
module.exports.getColaboradoresConsentimentosRespostaEmpresa = async function(req, res, next) {
  try{

    logger("SERVIDOR:ClientesId").info("Buscar cliente pelo Id")
    const {empresa_id} = req.params

    const results = await models.getColaboradoresConsentimentosRespostaEmpresa(empresa_id);
    res.status(results.statusCode).json(results)
...
module.exports.getColaboradoresConsentimentosResostaEmpresaColaborador = async function(req, res, next) {
  try{

    logger("SERVIDOR:ClientesId").info("Buscar cliente pelo Id")
    const {empresa_id, id_colaborador_de_consentimento_resposta} = req.params

    const results = await models.getColaboradoresConsentimentosResostaEmpresaColaborador(empresa_id, id_colaborador_de_consentimento_resposta);
    res.status(results.statusCode).json(results)
...
module.exports.getColaboradoresConsentimentosResostaEmpresaCategoria = async function(req, res, next) {
  try{

    logger("SERVIDOR:ClientesId").info("Buscar cliente pelo Id")
    const {empresa_id, categoria_da_pergunta} = req.params

    const results = await models.getColaboradoresConsentimentosResostaEmpresaCategoria(empresa_id, categoria_da_pergunta);
    res.status(results.statusCode).json(results)
...
module.exports.postColaboradoresConsentimentosRespostas = async function(req, res, next) { 
   ...
   try {
      ...
      const schemaColaboradoresConsentimentos = yup.object().shape({
        pergunta_catagorizada: yup.number().required(),
        resposta_categorizada: yup.number().required(),
        empresa_parametrizada: yup.number().required(),
        colaborador_consentimento: yup.number().required()
      })

      logger("SERVIDOR:postClientes").debug(`Á validar os dados ${JSON.stringify(dados)}`)
      const validar = await schemaColaboradoresConsentimentos.validate(dados)
      
      const result = await models.postColaboradoresConsentimentosRespostas(validar, req)  
