const models = require('../models/perguntas_categorias')
...
module.exports.getPerguntasCategorias = async function(req, res, next) {
  try{
      logger("SERVIDOR:Clientes").info("Buscar clientes")
      const {pagina, limite, categoria_id = '', pergunta = ''} = req.query
      const results = await models.getPerguntasCategorias(pagina, limite, categoria_id, pergunta); 
...
module.exports.getPerguntasCategoriasId = async function(req, res, next) {
  try{

    logger("SERVIDOR:ClientesId").info("Buscar cliente pelo Id")
    const {id_pergunta} = req.params

    const results = await models.getPerguntasCategoriasId(id_pergunta);
    res.status(results.statusCode).json(results)
...
module.exports.getPerguntasCategoriasPorCategoria = async function(req, res, next) {
  try{

    logger("SERVIDOR:ClientesId").info("Buscar cliente pelo Id")
    const {categoria_id} = req.params

    const results = await models.getPerguntasCategoriasPorCategoria(categoria_id);
    res.status(results.statusCode).json(results)
...
module.exports.postPerguntasCategorias = async function(req, res, next) { 
   ...
   try {
      ...
      const schemaPerguntasCategorias = yup.object().shape({
      ...
      const validar = await schemaPerguntasCategorias.validate(dados)
      
      const result = await models.postPerguntasCategorias(validar, req)  
...
module.exports.patchPerguntasCategorias = async function(req, res, next) { 
      try {
        ...
        const result = await models.patchPerguntasCategorias(id_pergunta, validar, req)
...
module.exports.deletePerguntasCategorias = async function(req, res, next) {
  try {

      logger("SERVIDOR:deleteClientes").info(`Iniciando a exlusão do cliente`)
      const {id_pergunta} = req.params
      const result = await models.deletePerguntasCategorias(id_pergunta, req)
